...
App.ChartStyle = (function () {
  ...
  /* 2026-08-31 대표 "내가 글씨 크게 하라는 거 다 크게해" (★세 번째 지시★) — 다시 확대
     제목 20 -> 24px / 닫기 ✕ 20 -> 24px / 왼쪽 탭 16 -> 19px(탭 열 144 -> 168)
     항목 이름 16 -> 19px / 아래 설명 14 -> 17px / 맨 아래 설명 14 -> 17px
     고른값 표시 14 -> 17px / 선택 상자 16 -> 19px(높이 40 -> 46, 최소폭 124 -> 140)
     체크박스 글자 16 -> 19px(네모 17 -> 20) / 버튼 16 -> 19px(높이 48 -> 54)
     색 타일 28 -> 32px(안 20 -> 23) / 줄 최소높이 44 -> 50px
     창 폭 620 -> 680px (max-width:100% 라 360 폰에서는 화면 폭을 따릅니다)
     되돌리려면 화살표 왼쪽 값으로. 색은 그대로입니다.
     (두 번째 지시 때의 값: 제목 20 / 이름 16 / 설명 14 / 버튼 16 / 폭 620) */
  function injectStyle() {
    ...
    var css =
      ...
      P + " .tl-cs-box{width:680px;max-width:100%;max-height:100%;background:" + C_CARD + ";" +
      ...
      "padding:13px 16px;border-bottom:1px solid " + C_BORDER + ";flex:0 0 auto;}" +
      P + " .tl-cs-title{font-size:24px;font-weight:700;color:" + C_TEXT + ";}" +
      P + " .tl-cs-x{background:none;border:0;color:" + C_MUTED + ";font-size:24px;line-height:1;" +
      ...
      P + " .tl-cs-tabs{width:168px;flex:0 0 168px;border-right:1px solid " + C_BORDER + ";" +
      "background:" + C_TILE + ";padding:8px 0;}" +
      P + " .tl-cs-tab{width:100%;display:block;background:none;border:0;text-align:left;" +
      "padding:13px 14px;font-size:19px;font-weight:600;color:" + C_MUTED + ";cursor:pointer;" +
      "font-family:inherit;}" +
      ...
      P + " .tl-cs-row{display:flex;align-items:center;gap:10px;min-height:50px;}" +
      P + " .tl-cs-label{flex:1 1 auto;min-width:0;font-size:19px;line-height:1.4;color:" + C_TEXT + ";}" +
      P + " .tl-cs-sub{display:block;font-size:17px;line-height:1.45;color:" + C_MUTED + ";margin-top:3px;}" +
      P + " .tl-cs-ctl{flex:0 0 auto;display:flex;align-items:center;gap:6px;}" +
      P + " select.tl-cs-sel{height:46px;min-width:140px;background:" + C_TILE + ";color:" + C_TEXT + ";" +
      "border:1px solid " + C_BORDER + ";border-radius:6px;font-size:19px;font-family:inherit;" +
      "padding:0 8px;cursor:pointer;}" +
      P + " .tl-cs-sw{width:32px;height:32px;border-radius:6px;background:" + C_TILE + ";" +
      "border:1px solid " + C_BORDER + ";display:inline-flex;align-items:center;justify-content:center;" +
      "cursor:pointer;padding:0;position:relative;}" +
      P + " .tl-cs-sw i{display:block;width:23px;height:23px;border-radius:4px;}" +
      P + " .tl-cs-sw input{position:absolute;left:0;top:0;width:100%;height:100%;opacity:0;" +
      "cursor:pointer;border:0;padding:0;}" +
      P + " .tl-cs-chk{display:inline-flex;align-items:center;gap:8px;font-size:19px;color:" + C_TEXT + ";" +
      "cursor:pointer;}" +
      P + " .tl-cs-chk input{accent-color:" + C_POINT + ";width:20px;height:20px;cursor:pointer;}" +
      P + " .tl-cs-note{font-size:17px;line-height:1.55;color:" + C_MUTED + ";padding-top:8px;" +
      ...
      P + " .tl-cs-btn{height:54px;min-width:124px;border-radius:6px;border:1px solid " + C_BORDER + ";" +
      "background:" + C_TILE + ";color:" + C_TEXT + ";font-size:19px;font-weight:600;cursor:pointer;" +
      "font-family:inherit;padding:0 16px;}" +
      P + " .tl-cs-btn.on{background:" + C_POINT + ";border-color:" + C_POINT + ";color:" + C_BG + ";}" +
      P + " .tl-cs-fixed{font-size:17px;color:" + C_MUTED + ";}" +
      ...
      P + " .tl-cs-tab{width:auto;flex:1 1 0;text-align:center;padding:12px 6px;}" +
      P + " .tl-cs-row{flex-wrap:wrap;min-height:0;padding:9px 0;gap:8px;}" +
      P + " .tl-cs-label{flex:1 1 100%;}" +
      P + " select.tl-cs-sel{min-width:118px;}" +
      "}";
