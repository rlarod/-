 ...
 *           ⚠️ 2026-09-04 에 같은 승인 안에서 ★27색★ 이 됐습니다(PM 판단).
 *              왜 늘렸는지 · 어떻게 골랐는지는 LINE_COLORS 안 주석에 있습니다.
...
App.ChartIndicatorKit = (function () {
  ...
  var LINE_COLORS = [
    ...
    /* ↓ 2026-09-04 (2건) 에 늘린 7색 - 20 -> ★27★.
       ⭐ 대표 승인은 2026-09-02 "차트 지표에 한해 색을 새로 만들어도 될까요" -> ★"ㅇㅋ"★.
          같은 승인 안에서 PM 이 2026-09-04 에 27색으로 정했습니다. 근거 -
          ★트레이딩뷰는 지표선을 전부 실선으로 그립니다.★ 선이 모자란다고 점선 ·
          파선으로 가르면 트레이딩뷰와 달라집니다(CLAUDE.md - 차트 시스템 전체는
          트레이딩뷰를 따라간다). 그래서 ★구분은 색으로만★ 하고 색을 늘렸습니다.
          확정 팔레트 9색은 ★한 값도 안 건드렸습니다.★ 늘어난 것은 이 목록뿐입니다.

       -- ⚠️ ★얹지 않고 27색 전체로 다시 뽑았습니다★ ------------------------
       기존 20색에 7색을 그냥 얹으면(= 새 색끼리는 안 재고 "기존 20색과 먼 것"
       일곱 개를 고르면) 새 색들이 ★한 자리에 뭉칩니다.★ 그때 27색 최소 ΔE2000 은
       9.71 -> ★0.68★ 로 무너집니다(2026-09-04 차트팀 실측. 9월 3일 시도에서는
       8.83 이었습니다 - 어느 쪽이든 바닥이 내려갑니다).
       그래서 한 개 고를 때마다 ★그때까지의 27색 전체★ 를 다시 재는 maximin 으로
       뽑았습니다.
       ★앞 20색의 값과 순서는 한 자도 안 바꿨습니다★ - 바꾸면 이미 쓰시던 분들의
       저장된 지표 색이 목록 밖 값이 되어 거부됩니다(아래 define/update 검사).

       -- 어떻게 골랐나 (눈대중 아님 · 전수 계산) ---------------------------
       RGB 격자를 전부 훑어 아래를 ★모두★ 통과한 것만 남기고, 남은 것 중에서
       27색 전체와의 최소 거리가 제일 먼 것부터 일곱 번 골랐습니다.

         조건 1  배경 #0A0F1C 과 명암비 4.60 이상  (새 7색 실측 최소 4.60)
                 27색 전체 바닥은 4.52(#4974E9 · 기존 색) 로 ★그대로★ 입니다
         조건 2  모든 색끼리 ΔE76 22.01 이상        (새 7색 실측 최소 22.33)
         조건 3  모든 색끼리 ΔE2000 9.71 이상       (새 7색 실측 최소 10.38)
         조건 4  상승 #26C281 · 하락 #F0506E 와 ΔE76 46.4 이상
                 (새 7색 실측 - 상승 최소 48.7 · 하락 최소 53.9)
                 ⭐ ΔE76 만으로는 모자랍니다. ★사람 눈에 가까운 ΔE2000 으로도★
                    지금 20색의 바닥 이상을 지키게 했습니다 -
                      기존 20색   상승 최소 24.5(#E1ED97) · 하락 최소 19.4(#BB81AC)
                      새 7색      상승 최소 24.5          · 하락 최소 23.8
                    ⚠️ 이 조건을 빼면 hue 319 짜리 진분홍(#FF01AE 계열)이 통과합니다.
                       ΔE76 로는 46.4 로 합격인데 ΔE2000 으로는 하락색과 16.4 뿐이라
                       ★지금 어느 지표색보다도 하락색에 가깝습니다.★ 그래서 뺐습니다
         조건 5  ★★그림선 8색 · 알람색 1색과도 ΔE2000 10.5 이상★★
                 js/chart-drawings.js 의 DRAW_COLORS · ALERT_COLOR 입니다.
                 회원 눈에는 지표선이나 추세선이나 ★그냥 차트 위의 선★ 이라
                 무리를 넘어서도 겹치면 안 됩니다(tests/chart-color-namespace-seal).
                 ⚠️ 이 조건을 안 걸면 그림색 ★살구★ · ★연분홍★ 과 거의 같은
                    색이 통과합니다. 실제로 처음 뽑은 두 색이 그래서 탈락했습니다.
                 새 7색 실측 최소 11.24 (#907C28 - 그림색 겨자)
                 ⚠️ 이 주석에 ★그림색 값을 적지 마세요★ -
                    tests/chart-drawings.test.js 가 이 목록 구간의 색값을
                    ★주석까지 통째로★ 긁어 견줍니다. 적으면 거리 0 으로 터집니다
         조건 6  색상환에서 초록 · 빨강 쪽으로 물러났습니다 - ★24~75도와
                 190~325도★ 안에서만 골랐습니다. 봉인이 막는 구간은 초록
                 100~185 · 빨강 330~18 인데, 계산만 믿으면 hue 98(형광 연두) ·
                 18(주홍) 이 통과합니다. 숫자로는 멀어도 ★회원 눈에는 손익색★ 입니다.
                 ⚠️ 2026-09-02 에는 24~62 · 195~320 이었습니다. 조금 넓혔습니다 -
                    이미 배포된 #E1ED97(hue 68) · #49C9E9(hue 192) 가 그 밖에
                    있어서 62 · 195 라는 금이 실제와 맞지 않았습니다.
                    새 7색의 실제 hue 는 48.5 · 50.0 · 67.8 · 74.8 · 75.0 ·
                    251.8 · 292.5 로, 초록 구간과 25.0도 · 빨강 구간과 37.5도
                    떨어져 있습니다.

       -- 27색 전수 실측 (2026-09-04 · tests 절 1 이 매번 다시 잽니다) ------
            최소 ΔE76    22.01  (#4974E9 / #9197F3   - ★기존 쌍 그대로★)
            최소 ΔE2000   9.71  (#BA6EED / #E637E6   - ★기존 쌍 그대로★)
            최소 명암비    4.52  (#4974E9            - ★기존 색 그대로★)
            지표27 <-> 그림·알람9 최소 ΔE2000 10.48 (#FF8F3C 주황 - 그림색 진주황.
                                                    ★기존 쌍 그대로★)
       ★세 바닥값이 20색일 때와 한 자도 다르지 않습니다.★ 새 7색은 전부 그보다
       멀리 있습니다 - 새 색이 낀 쌍의 최소는 ΔE2000 10.38 (#A862B2 / #BA6EED).
       손익색과의 거리도 ★오늘 바닥보다 나빠지지 않았습니다★ (위 조건 4 표 참조).
       ★다음에 터질 자리는 여전히 보라(#BA6EED)와 자홍(#E637E6) 입니다.★

       ★순서를 뒤에 붙였습니다★ - suggestColor() 가 앞에서부터 고르기 때문에,
       앞에 끼워 넣으면 이미 쓰시던 분들의 자동 색이 바뀝니다. */
    { key: "orchid", hex: "#A862B2", name: "진자주" },
    { key: "violet", hex: "#8061FF", name: "청보라" },
    { key: "lime", hex: "#B2CC04", name: "연두" },
    { key: "neon", hex: "#C0FF00", name: "형광연두" },
    { key: "olive", hex: "#907C28", name: "진카키" },
    { key: "ash", hex: "#B0AEA4", name: "잿빛" },
    { key: "moss", hex: "#788060", name: "진올리브" }
  ];
