...
function boot(opts) {
  ...
  [/* js/risk-brackets.js — 2026-08-31 대표 결재(바이낸스 구간별 유지증거금). index.html 은 risk-brackets → trading 순서라 여기도 같게 태웁니다. 안 태우면 이 테스트는 회원이 겪지 않는 옛 고정값(MMR_FALLBACK 0.5%) 경로를 재게 됩니다. */
   "js/config.js", "js/utils.js", "js/storage.js", "js/risk-brackets.js", "js/trading.js", "js/max-margin-safe.js", "js/rank.js", "js/notice-board.js", "js/user-panel.js", "js/ad-slots.js", "js/chat-event-style.js", "js/theme.js", "js/board-gallery-style.js", "js/board-paging.js"].forEach((f) => {
...
section("[5] 기존 기능 보존");
{
  ...
  /* 2026-08-27 기준 갱신 — 원래 이 검사는 "상단 박스 3개 높이 통일" 이었습니다.
     막으려던 것은 `내 정보` 박스가 로그인/로그아웃에 따라 높이가 출렁여
     윗줄 정렬이 깨지는 것이었고, **그 목적은 오른쪽 박스 하나만 고정하면
     달성됩니다.**

     왼쪽까지 같은 높이로 묶었더니 로그아웃 상태 로그인 폼(293px) 때문에
     잡은 340px 이 왼쪽에도 강제됐는데, 왼쪽 실제 내용은 206px 이라
     134px 이 순수한 빈 칸이었습니다. 그만큼 차트가 아래로 밀렸습니다.

     본부장 실측 (2026-08-27)
       1920x1080 첫 화면 차트 높이   338.3px → 472.3px
       1800/1799 경계 절벽           183px   →  47.4px
       1440 / 360                    187.6 / 294.1 (변화 없음)

     그래서 기준을 이렇게 바꿉니다 — **약해지지 않게 한 줄을 더 넣습니다.**
       · 오른쪽 `내 정보` 는 여전히 height 고정 (원래 목적 그대로)
       · 왼쪽 공지/게시판은 height:auto + max-height:var(--top-box-h)
         max-height 가 핵심입니다. 공지는 서버에서 최대 20줄까지 내려올 수
         있어(js/notice-board.js:40) height:auto 만 두면 박스가 계속 길어져
         차트를 다시 밀어냅니다. */
  t("상단 박스 높이 — 내 정보는 고정, 공지/게시판은 내용 따라 (상한 있음)", () => {
    const css = fs.readFileSync(path.join(REPO, "style.css"), "utf8");
    ...
    /* ① 오른쪽 `내 정보` 는 높이가 고정 — 로그인/로그아웃으로 출렁이면 안 됩니다 */
    ok(/\.page-right > \.user-panel-box\{height:var\(--top-box-h\);\}/.test(css),
      "내 정보 박스가 --top-box-h 로 고정돼야 함 (로그인 폼이 들어오면 높이가 출렁임)");

    /* ② 왼쪽 공지/게시판은 내용 높이를 따르되 **상한이 있어야** 합니다.
          상한이 없으면 공지 20줄에 박스가 늘어나 차트를 다시 밀어냅니다. */
    const 왼쪽 = css.match(/\.notice-board-wrap > \.notice-box\{([^}]*)\}/);
    ok(왼쪽, "공지/게시판 박스 높이 규칙이 있어야 함");
    ok(왼쪽 && /height:\s*auto/.test(왼쪽[1]),
      "공지/게시판은 내용 높이를 따라야 함 (고정하면 빈 칸이 차트를 밀어냄): " + (왼쪽 && 왼쪽[1]));
    ok(왼쪽 && /max-height:\s*var\(--top-box-h\)/.test(왼쪽[1]),
      "상한이 없으면 공지가 20줄까지 늘어나 차트를 다시 밀어냅니다(js/notice-board.js:40): " +
      (왼쪽 && 왼쪽[1]));
  ...
  t("헤더: 레퍼런스 구조(로고 + [공지] 한 줄 + 우측 상태)와 컴팩트한 크기", () => {
    ...
    ok(size(/\.brand \.name\{[^}]*font-size:([\d.]+)px/, "로고 이름") >= 26, "로고 이름이 레퍼런스보다 작음");  /* 2순위(2026-08-25): 옛 하한 26 -> 15 */
