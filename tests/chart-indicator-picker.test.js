...
절("[2] ★팝업 글씨 바닥값 17px★ (한도 0)");
{
  ...
  /* ───────────────────────────────────────────────────────────────────
   * ⚠️ 2026-09-04 기록팀 — ★위 검사는 px 라고 적힌 것만 셉니다.★
   *
   *   1.0625rem(=17px) · 1em · 120% · 13pt · 4vw · calc() · clamp() 로 적으면
   *   위 while 문이 ★한 번도 안 돕니다.★ 값이 작아져도 "0곳" 이라 초록입니다.
   *   대표가 글씨 크기로 네 번 지적하신 자리라 이 구멍이 실제로 아픕니다.
   *
   *   ★실측 (사본에서 · 진짜 파일은 안 건드렸습니다)★
   *     js/chart-indicator-kit.js 사본의 .tl-kit-btn 앞에
   *         font-size:clamp(11px, 2vw, 17px)    ← 360 에서는 11px 로 그려집니다
   *     를 끼웠더니 옛 방식은 ★옆 규칙의 17px 을 대신 읽어★ 17 이라 보고했고
   *     "17px 미만 0곳" 그대로였습니다. 0.6875rem(=11px) 도 같았습니다.
   *
   *   ★정규식을 늘리지 않고 값을 판정합니다.★ (색 봉인 [3] 과 같은 방식입니다)
   *   표기를 하나씩 막으면 하나 늘 때마다 구멍이 하나 생깁니다.
   *   판정은 tests/_font-size.js 한 곳 — 근거·예외 두 가지는 그 파일 머리말에.
   * ─────────────────────────────────────────────────────────────────── */
  const 검 = 글씨단위.자체검증();
  ok("단위 판정기가 표본 " + 검.표본수 + "개를 다 맞춘다 (tests/_font-size.js)",
    검.전부통과, 검.설명);
  const 단위위반 = 글씨단위.단위위반(알맹이);
  ok("font-size 를 px 로만 적었다 (rem · em · % · vw · clamp · calc 금지)",
    단위위반.length === 0, 글씨단위.요약(단위위반));
  const 선언수 = 글씨단위.선언들(알맹이).length;
  ok("font-size 선언을 5개 이상 읽었다 (검사가 헛돌지 않았다 · " + 선언수 + "개)",
    선언수 >= 5);
}
