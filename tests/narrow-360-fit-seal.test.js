 ...
 *     자리   style.css 4884행 @media(max-width:400px)
 ...
 *
 *   ⚠️ 401~700px 구간은 이 봉인이 15 로 잠그지 않습니다. 왜 그런지 —
 *      08e93b0 이 올린 것은 @media(max-width:400px) 뿐이라, 401~700px 은
 *      style.css 484행의 13.5px 이 그대로 남았습니다. 360 보다 넓은 폰이
 *      글자가 더 작은 ★역전★ 입니다.
 *      ★이 파일을 고치던 2026-09-07 그 시각에 디자인팀이 바로 그 구간을
 *        고치고 있었습니다.★ 실제로 검사를 돌리는 사이에 484행이 13.5 → 15 로
 *        바뀌는 것을 봤습니다(작업트리 기준. 아직 커밋 전입니다).
 *      그래서 아래 밴드 표는 401~1920 의 바닥을 ★예전 값 13 그대로★ 둡니다.
 *      15 로 잠그면 디자인팀 작업이 반려됐을 때 이 봉인이 같이 빨개집니다.
 *
 *      ★다음 사람에게★ — 디자인팀의 401~700 15px 이 게이트 2 를 통과해
 *      커밋되면, 아래 밴드 표 두 번째 줄의 `바닥: 13` 을 `바닥: 15` 로 올리세요.
 *      그때 이 파일에 "언제 어느 커밋으로 올렸는지" 를 같이 적으세요.
 *      (2026-09-04 에 여기 13 을 남겨 둔 탓에 사흘간 13px 되돌림을 못 잡았습니다.
 *       같은 일을 반복하지 않으려고 조건과 자리를 콕 집어 적어 둡니다.)
 *
...
console.log("\n  [4] 상단 메뉴 - 바닥은 " + 바닥 + "px, 천장은 '360 에서 한 줄에 읽히는가'");
{
  ...
  /* (가-2) 폭6 사이의 빈틈 - 새 미디어쿼리로 몰래 낮추는 것 방지.
     ...
     잡았습니다. 바닥이 13 일 때는 style.css 484행의 13.5px 이 통과해서
     아무 문제가 없었는데, 바닥을 15 로 올리자 ★그 13.5px 이 걸렸습니다.★

     그런데 484행은 잘못된 규칙이 아닙니다. @media(max-width:700px) 라
     360px 에서도 "적용은 되지만" 뒤에 오는 4884행 @media(max-width:400px) 의
     ...
     밴드를 나눈 이유:
       320~400   이 봉인이 지키는 구간. 2026-09-04 에 15px 이 들어왔습니다(08e93b0)
       401~1920  ★이 봉인의 주제가 아닙니다.★ 지금 401~700 은 13.5px 이고
                 디자인팀이 역전을 손보는 중입니다. 예전 바닥 13 을 그대로 둡니다 -
                 올리는 것(15px)은 막지 않고, 더 낮추는 것만 막습니다.
     ──────────────────────────────────────────────────────────────────────── */
  const 밴드 = [
    { 이름: "320~400px (이 봉인이 지키는 구간)", 부터: 320, 까지: 400, 바닥: 바닥 },
    { 이름: "401~1920px (주제 밖 - 예전 바닥 13 유지)", 부터: 401, 까지: 1920, 바닥: 13 }
  ];
...
console.log("\n  [돌연변이] 하나씩 되돌리면 정말 실패하는가");
{
  ...
  /* (자) ★15px + 여백 3px 조합이 이 봉인을 통과한다★
         ...
         2026-09-07 현재 그 안이 08e93b0 으로 실제로 들어왔습니다(style.css 4884행).
