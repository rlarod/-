...
const 엔진필수 = [
  {
    ...
    /* ⚠️⚠️ 2026-09-01 — ★지금은 앞뒤가 뒤바뀐 상태입니다. 읽고 넘어가세요.★
         대표 "상한을 없애면되네 금액별로 상한 없애버려 몇백억 있어도 배율 자유롭게 쓸수있게"
       → index.html 에서 <script src="js/risk-brackets.js"> 한 줄을 주석 처리했습니다(4eed5d6).
       → 라이브 회원은 지금 ★유지증거금 0.5% 고정 · 배율 상한 없음★ 을 겪습니다.

       그런데 이 목록은 여전히 구간표를 태웁니다. 그래서 위 '이유' 가 지금은 거꾸로입니다 —
         원래: 안 태우면 → 테스트가 회원이 안 겪는 옛 값을 잰다
         지금: ★태우면★  → 테스트가 회원이 안 겪는 새 값을 잰다

       ⭐ 그래도 ★일부러 그대로 둡니다.★ 대표가 다시 켜라고 하시면 한 줄로 켜야 하고,
          그때 봉인 40여 개가 살아 있어야 하기 때문입니다. PM 이 알고 있는 상태입니다.
       ⚠️ 이 상태를 바꾸려면(= 목록에서 빼서 라이브와 맞추려면) 봉인 수십 개가
          한꺼번에 빨강이 됩니다. ★PM 결정 사항이고, 기록팀이 임의로 안 바꿉니다.★
       ⭐ '지금 꺼져 있다' 는 사실 자체는 tests/mmr-fallback-blindspot.test.js [4] 와
          tests/risk-brackets-tiered-mmr.test.js 7절이 못 박습니다 — 다시 켜면 거기서 터집니다. */
    라이브_2026_09_01: "꺼져 있음 (index.html 에서 주석 처리 · 대표 지시 4eed5d6)",
  },
];
