...
function boot(opts) {
  ...
  files.push("js/risk-brackets.js");
  files.push("js/trading.js");
  files.push("js/max-margin-safe.js");   // 엔진을 감쌉니다 — 반드시 trading.js 뒤
...
/* =========================================================================
 * [15] 그물이 "이름"이 아니라 "거래엔진"을 고른다
 * -------------------------------------------------------------------------
 * 2026-08-27 실측으로 잡힌 것 — onPriceUpdate 라는 이름을 가진 price:update
 * 구독자가 5개입니다. 이름만으로 고르면 차트·호가·전황까지 같이 걸러져서,
 * 그물이 켜지는 순간 화면이 통째로 얼어붙습니다(시세 53틱 → 265건 = 5배).
 * 그물은 거래엔진 하나만 감싸야 합니다.
 * ========================================================================= */
section("[15] 그물은 거래엔진 하나만 감싼다");
{
  /* (1) 이름이 실제로 겹친다는 사실을 못 박아 둡니다. */
  const JS_DIR = path.join(REPO, "js");
  const 이름겹침 = fs.readdirSync(JS_DIR)
    .filter((f) => f.endsWith(".js") && f !== "symbol-guard.js")
    .filter((f) => /function\s+onPriceUpdate\s*\(/.test(read("js/" + f)));
  ok("onPriceUpdate 라는 이름의 구독자가 2개 이상이다(이름으로 고르면 안 된다)",
    이름겹침.length >= 2, 이름겹침.join(", "));
  console.log("      └ 같은 이름: " + 이름겹침.join(", "));

  /* (2) 지문으로 거래엔진만 가려내는가 */
  const { App } = boot();
  ok("그물을 씌운 구독자가 정확히 1개다(거래엔진)",
    App.SymbolGuard.getNettedCount() === 1,
    "지금 " + App.SymbolGuard.getNettedCount() + "개 — 1이 아니면 화면 구독자까지 걸렀거나 못 걸렀습니다");

  /* (3) 이름만 같은 화면쪽 구독자는 그물 밖이어야 합니다. */
  let 화면본틱 = 0;
  function onPriceUpdate(payload) { // 차트·호가와 같은 이름, 다른 본문
    if (payload && payload.symbol === "ETHUSDT") 화면본틱++;
  }
  App.Bus.on("price:update", onPriceUpdate);
  ok("이름만 같은 화면쪽 구독자는 그물을 씌우지 않는다",
    App.SymbolGuard.getNettedCount() === 1, String(App.SymbolGuard.getNettedCount()));

  ok("거래엔진 본문은 지문에 걸린다",
    App.SymbolGuard.isEngineHandler(function onPriceUpdate(p) { checkPendingOrder(); checkTriggers(); }) === true);
  ok("화면쪽 본문은 지문에 안 걸린다",
    App.SymbolGuard.isEngineHandler(onPriceUpdate) === false);

  /* (4) 실제로 다른 종목 시세를 흘려보냅니다. */
  const opened = openLong(App, 110000, 1000, 10);
  ok("BTC 포지션을 열었다", opened.ok === true, opened.error || "");
  App.SymbolGuard._reset();
  화면본틱 = 0;

  for (let i = 0; i < 10; i++) {
    App.Bus.emit("price:update", { symbol: "ETHUSDT", price: 3000, time: Date.now() });
  }

  const s = App.Trading.getSnapshot();
  ok("포지션이 살아 있다(강제청산 안 됨)", !!s.position);
  ok("그물이 버린 건수가 흘린 건수와 같다(10건, 5배로 부풀지 않는다)",
    App.SymbolGuard.getDroppedCount() === 10,
    "버린 건수 " + App.SymbolGuard.getDroppedCount() + " — 흘린 건 10건입니다");
  ok("화면쪽 구독자는 10건을 그대로 받았다(차트·호가가 안 얼어붙는다)",
    화면본틱 === 10, "받은 건수 " + 화면본틱);
}
