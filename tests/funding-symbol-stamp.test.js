...
/* ===================================================================== */
console.log("\n  (6) 화면 쪽 — js/ui.js 는 그대로고, 우회 모듈이 덮습니다");
/* ===================================================================== */
{
  /* ── 2026-08-28 문구 갱신 (수리팀 보고 / 기록팀 처리) ────────────────
     예전에 여기 적혀 있던 말: "우회 모듈이 아직 없음을 기록".
     그때는 사실이었지만 지금은 아닙니다 — 2026-08-27 밤에
     js/funding-qty-symbol.js 가 생겼습니다(DOM 후처리로 덮어씁니다).
     검사 결과는 그때나 지금이나 같지만 ★문구만 사실과 달랐습니다.★
     읽는 사람이 "아직 안 고쳐졌구나" 로 오해하므로 고칩니다.

     js/ui.js 는 여전히 수정 금지라 f.symbol 을 안 씁니다. 그건 정상이고,
     그대로여야 합니다. 그래서 아래 두 가지를 같이 봅니다 —
       ① js/ui.js 는 여전히 종목을 안 넘긴다  (우회가 계속 필요하다는 뜻)
       ② 그걸 덮는 우회 모듈이 실제로 있고 index.html 에 실려 있다
     ①만 있고 ②가 사라지면 화면이 조용히 옛 버그로 돌아갑니다. */
  ...
  console.log("     · js/ui.js 가 종목을 넘기나: " +
    (종목을주나 ? "예" : "아니오 — 수정 금지 파일이라 그대로입니다"));

  ok("js/ui.js 는 수정 금지라 그대로다 (그래서 우회가 필요합니다)",
    !종목을주나, 그줄 ? 그줄[0] : "(못 찾음)");

  /* ② 그 줄을 덮는 우회 모듈 — 2026-08-27 밤에 생겼습니다 */
  let 우회있다 = false;
  try { 우회있다 = read("js/funding-qty-symbol.js").length > 0; } catch (e) { 우회있다 = false; }
  ok("우회 모듈 js/funding-qty-symbol.js 가 있다", 우회있다,
    "없으면 화면이 다시 활성 종목 단위로 떨어집니다");
  const html = read("index.html");
  ok("그 우회 모듈이 index.html 에 실려 있다",
    /<script src="js\/funding-qty-symbol\.js"><\/script>/.test(html),
    "파일만 있고 안 실리면 아무 일도 안 합니다");
  console.log("     · 덮는 모듈: js/funding-qty-symbol.js (자세한 검사는 tests/funding-qty-symbol.test.js)");
}
