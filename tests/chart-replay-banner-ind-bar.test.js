...
/* =====================================================================
 * 8) 글씨를 안 줄였다 — 안내줄·도움말 글씨 크기
 *
 * ⚠️ 2026-09-03 수리팀 — 여기가 원래 "=== 16" 이었습니다.
 *    이 절의 뜻은 제목 그대로 ★안 줄였다★ 인데, 같다(===) 로 적어서
 *    ★천장★ 이 돼 있었습니다. 대표가 "글씨를 키워라" 라고 하신 것이
 *    네 번째라 16 → 17 로 올렸더니 이 두 줄이 터졌습니다.
 *    뜻을 그대로 두고 ★바닥(>=)★ 으로 고칩니다 — 15px 로 줄이면 여전히 빨갛고,
 *    더 키우는 것은 안 막습니다.
 *    ⚠️ 다시 "=== 숫자" 로 되돌리지 마세요. 대표가 또 키우라고 할 때마다
 *       이 파일을 고쳐야 하고, 그게 지난 세 번 글씨가 다시 작아진 경로입니다.
 *    바닥값 자체는 tests/chart-replay-font-floor.test.js 가 17px 로 지킵니다.
 * ===================================================================== */
{
  const 최소 = 16;
  const 안내 = /\.tl-rp-banner\{[^}]*font-size:(\d+)px/.exec(SRC);
  const 도움말 = /\.tl-rp-hint\{[^}]*font-size:(\d+)px/.exec(SRC);
  ok("안내줄 글씨를 안 줄였다 (" + 최소 + "px 이상)", 안내 && Number(안내[1]) >= 최소, 안내 ? 안내[1] : "없음");
  ok("도움말 글씨를 안 줄였다 (" + 최소 + "px 이상)", 도움말 && Number(도움말[1]) >= 최소, 도움말 ? 도움말[1] : "없음");
}
