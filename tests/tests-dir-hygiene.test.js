...
const 도우미목록 = {
  ...
  /* 2026-09-04 — 글씨 바닥값(17px) 봉인 9개가 ★px 라고 적힌 것만★ 세고 있었습니다.
     1.0625rem · 1em · clamp(11px,2vw,17px) 로 적으면 검사를 통째로 빠져나갑니다.
     판정을 9곳에 베끼면 한 곳이 반드시 낡습니다(해시 48곳과 같은 사고).
     여기 한 곳에 두고, tests/font-size-unit-source.test.js 가 이 한 곳을 지킵니다. */
  "_font-size.js": "font-size 단위 판정기(글씨 바닥값 봉인 9개가 읽습니다)",
  /* 2026-09-07 — 주문창 "과도한 볼드 금지" 검사가 /font-weight:(800|900)/ 한 줄이라
     ★글자 모양 두 개만★ 봤습니다. 실측 — `font-weight: 800`(공백 하나) ·
     `bolder` · `850` · 대문자 표기가 ★넷 다 그냥 통과★ 였습니다.
     반대로 주석 산문의 굵기까지 값으로 읽어 거짓 실패도 낼 수 있었습니다.
     _font-size.js 와 같은 이유·같은 모양으로 판정을 한 곳에 둡니다.
     tests/font-weight-value-source.test.js 가 이 한 곳을 지킵니다. */
  "_font-weight.js": "font-weight 굵기 판정기(주문창 볼드 상한 봉인이 읽습니다)",
};
...
section("[2] 정체불명 파일이 커밋에 들어가 있나");
{
  ...
  /* ⚠️ 상한을 올릴 때는 ★왜 늘렸는지★ 를 여기 같이 적습니다. 그냥 올리면
     "안 되면 상한을 올린다" 가 관행이 되어 이 검사가 죽습니다.
       8 → 9  (2026-09-04 기록팀) _font-size.js. 글씨 바닥값 봉인 9개가 저마다
              /font-size:\s*(\d+)px/ 를 들고 있어서 rem·clamp 를 통째로 놓쳤습니다.
              판정을 9곳에 베끼면 반드시 한 곳이 낡습니다(md5 48곳과 같은 사고).
       9 → 10 (2026-09-07 기록팀) _font-weight.js. 같은 병입니다 — 주문창 볼드
              상한이 /font-weight:(800|900)/ 한 줄이라 `font-weight: 800`(공백)·
              bolder·850·대문자를 ★넷 다 놓쳤습니다★(실측). 지금은 읽는 쪽이
              하나뿐이지만, 판정 로직을 봉인 안에 인라인으로 두면 다음 사람이
              그대로 베낍니다. 글씨 크기 구멍이 9곳으로 번진 경로가 정확히 그것이라
              ★한 곳에 두고 봉인을 붙이는★ 쪽을 골랐습니다. */
  ok("도우미목록이 불필요하게 커지지 않았다 (지금 " + 도우미이름.length + "개, 상한 10)",
    도우미이름.length <= 10,
