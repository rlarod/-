 ...
 *      칩(.tl-draw-chip · .tl-zoom-chip)은 CSS 가 white-space:nowrap 이라
 *      한 줄에서 안 늘어납니다. 그래서 ㉯ 에서 뺍니다.
 *      ★그 면제 근거도 6절이 CSS 에서 직접 읽어 확인합니다★ — 누가 nowrap 을
 *      빼면 면제가 사라지고 그 자리에서 빨개집니다.
 *
 * ── 되돌리는 방법 ─────────────────────────────────────────────────────
...
const 도우미 = ["vpW", "vpH", "chipFloorY", "visibleBox", "putFixed"];
{
...
function 돌리기(대상, 화면, 배수, 표시, 소스변형) {
  ...
  vm.createContext(sb);
  let code = "var CHIP_EDGE=" + CHIP_EDGE + ";var LIST_MAX_H=" + LIST_MAX_H +
...
function 안접힘(키) {
  ...
  if (i < 0) return false;
  return SRC.slice(i, i + 400).indexOf("white-space:nowrap") >= 0;
}
...
/* =====================================================================
 * [6] 면제 근거 — 칩을 4절 ㉯ 에서 뺀 이유가 아직 사실인가
 * ===================================================================== */
절("[6] 면제 근거 — 칩은 CSS 가 한 줄로 묶어 두었다");
["chip", "zoomChip"].forEach((k) => {
  ok(크기표[k].선택자 + " 가 white-space:nowrap 이다 (그래서 칸보다 커질 수 없습니다)",
    안접힘(k),
    "★nowrap 이 없어졌습니다.★ 칩이 두 줄이 되면 placeChips 의 위 막기가 " +
    "칩을 도로 아래로 밀어 주문 막대에 물립니다 — 면제를 없애고 방어를 넣어야 합니다");
});
{
  /* 근거가 사라졌을 때 진짜로 문제가 되는지 숫자로 보여 둡니다(면제의 대가) */
  const r = 돌리기("placeChips", 화면들[0], 3, "flex");
  const x = r.잰것.filter((e) => e.키 === "chip")[0];
  ok("(참고) 칩이 세 줄(키 " + (x ? x.키높이 : "?") + ")이 되면 " +
    (x ? x.아래 - r.vis.bottom : "?") + "px 만큼 주문 막대 밑으로 내려갑니다",
    !!x && x.아래 - r.vis.bottom > 0,
    "이 참고 줄이 실패하면 오히려 좋아진 것입니다 — 면제를 없애도 됩니다");
}
