...
/* putFixed 를 안 쓰고 style.top 을 직접 정하는 함수도 세어 둡니다.
   ...
   ★둘 다 아래쪽 방어가 있습니다.★
     ...
     openFacePicker  차트 칸 바닥(wr.height) + chipFloorY() 로 막습니다
     openTextInput   같은 규칙으로 막습니다 —
                     js/chart-drawings.js 의 `var i바닥 = wr2.height - 4` 부터
                     `if (itop + ih > i바닥) itop = i바닥 - ih;` 까지 (2026-09-04
                     기준 6074~6081줄. ★줄 번호는 밀립니다 — 이름으로 찾으세요★)

   ⚠️ 2026-09-04 정정 — 여기 원래 "openTextInput 은 회원이 누른 자리에 붙는
      입력칸이라 방어가 없습니다" 라고 적혀 있었습니다. ★사실이 아닙니다.★
      2026-09-03 17차 수리 때 방어가 들어갔는데 이 주석만 그 전 상태로 남았습니다.
      다음 사람이 "무방비 구역" 으로 읽고 ★있는 방어를 또 넣을 자리★ 였습니다.
      (감사팀 지적 → 기록팀 정정. 코드는 멀쩡했고 주석만 틀렸습니다)

   어느 봉인이 실제로 검사하나 — ★행동으로 재는 것은 이 파일이 아닙니다★
     tests/chart-pane-popup-bottom-fit.test.js
       · 여섯 폭에서 스크롤을 훑어 최악 자리를 찾아 실제로 띄워 보고 잽니다
       · [7]절이 "입력칸이 chipFloorY() 를 본다" · "wr2.top 을 뺀다" 를 봉인합니다
       · 감사팀이 그 방어를 지워 보니 여섯 폭이 전부 빨개졌습니다 (2026-09-04 확인)
     이 파일은 ㉠ putFixed 쪽만 봅니다. 여기서는 ★아래 (견줌) 두 줄로 최소한★
     "방어가 통째로 사라지지 않았는지" 만 확인합니다.
  ...
  "지금: " + 칸좌표.join(",") + " → 새로 생겼으면 아래쪽 방어가 있는지 사람이 보세요");
{
  ...
  /* 2026-09-04 추가 — 위 주석이 "방어가 없다" 고 잘못 적혀 있던 자리입니다.
     주석만 고치면 다음에 또 틀어집니다. ★검사로 못 박아 둡니다.★
     행동으로 재는 것은 chart-pane-popup-bottom-fit 이고, 여기서는
     "방어가 통째로 사라지지 않았는지" 만 봅니다. */
  /* ⚠️ ★주석을 지우고 봅니다.★ 그냥 보면 바로 위 주석에 적힌
     "chipFloorY() 는 화면 기준…" 이라는 ★설명 글자★ 에 걸려서,
     정작 코드에서 chipFloorY 를 지워도 초록으로 통과합니다.
     2026-09-04 에 실제로 확인했습니다 — 주석을 안 지우면 이 검사는
     "방어를 지웠다" 를 못 잡습니다(돌연변이 2 가 초록으로 지나갔습니다). */
  const t = (함수떼기("openTextInput", SRC) || "")
    .replace(/\/\*[\s\S]*?\*\//g, " ")
    .replace(/(^|[^:])\/\/[^\n]*/g, "$1 ");
  ok("(견줌) openTextInput ★에도★ 아래쪽 방어가 있다 (chipFloorY 로 막는다)",
    /chipFloorY\(\)/.test(t) && /itop\s*\+\s*ih\s*>/.test(t),
    t.trim() ? "방어가 사라졌습니다 — 폰에서 입력칸이 매수·매도 막대 밑으로 내려갑니다" :
      "openTextInput 함수를 못 찾았습니다 (이름이 바뀌었나요)");
}
