 ...
 *
 * ── ⚠️ 훑는 범위에 ★하이픈 없는 js/chart.js★ 는 일부러 안 넣습니다 ────
 *   무늬가 /^chart-.*\.js$/ 라 js/chart.js 는 하이픈이 없어 안 들어옵니다.
 *   2026-09-08 에 가격축을 덮은 범인이 바로 그 파일이라("거래량 배지" P1),
 *   "범위를 넓혀야 하지 않나" 는 물음이 나왔습니다. ★재보고 안 넓혔습니다.★
 *
 *   ★2026-09-09 기록팀 실측★ — 넓힌 사본(/^chart.*\.js$/)을 실제로 돌렸습니다.
 *     훑는 파일   31개 → 32개 (js/chart.js 하나 늘어남)
 *     절대배치 규칙  21개 → ★21개 (한 개도 안 늘어남)★
 *     오른쪽없음 0 → 0 · 넘침없음 0 → 0 · 정적right 2 → 2
 *     검사 결과   32개 통과 / 0개 실패 → ★똑같이 32 / 0★
 *   까닭 — js/chart.js 는 519줄인데 ★CSS 를 아예 안 넣습니다★.
 *     position:absolute ★0건★, <style> 만들기 0건, style.cssText 0건.
 *     이미 index.html 에 있는 칸의 textContent 만 갈아 끼웁니다.
 *   ★넓혀도 영원히 0건입니다.★ 게다가 그 파일은 수정 금지 12개라
 *   설령 걸려도 못 고칩니다 — 켜자마자 끄고 싶은 봉인이 됩니다.
 *
 *   ⭐ 더 중요한 것 — ★2026-09-08 의 그 병은 CSS 규칙이 아닙니다.★
 *     addSeries 에 lastValueVisible / priceLineVisible 을 안 적어
 *     라이브러리 기본값이 켜진 것이고, 배지는 ★캔버스에 ctx.fillText 로★
 *     찍힙니다. ★선택자도 CSS 규칙도 없습니다.★
 *     범위를 아무리 넓혀도 이 훑개로는 영원히 안 잡힙니다 — 도구가 다릅니다.
 *     그 구멍은 tests/chart-series-axis-badge-seal.test.js 가 막습니다
 *     (2026-09-09 · 소스에 그 옵션을 적었는가 를 봅니다).
 *
