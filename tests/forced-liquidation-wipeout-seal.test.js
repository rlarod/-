...
}
{
  /* ★다른 구간에서도 같은 성질이 사는지★ 를 봅니다. 이게 이 블록의 목적입니다.
     ...
     ⚠️ 50배에서 지갑 130,000 을 쓰면 안 됩니다 — 아래 1-3 참조(최대 버튼의
        부동소수점 경계에 걸려 "가용 자산보다 큽니다" 로 거절됩니다). */
  const b = 한판(50000);
...
/* ========================================================================
 * 1-3. ⚠️ 발견 — 최대 버튼 값이 ★부동소수점 때문에 거절되는 지갑★ 이 있다
 * ------------------------------------------------------------------------
 * ⛔ 이건 "이게 옳다" 고 못 박는 게 아닙니다. ★지금 그렇다는 사실★ 을 적어두는
 *    것입니다. 고쳐지면 이 검사가 실패하면서 "이제 고쳐졌다" 고 알려줍니다.
 *
 * 무엇을 봤나 (2026-08-31 실측, 50배 · 최대 버튼)
 *   지갑  50,000 → 열림      지갑 100,000 → 열림      지갑 200,000 → 열림
 *   지갑 120,000 → ★거절★   지갑 130,000 → ★거절★   지갑 240,000 → ★거절★
 *   거절 사유는 전부 "증거금과 수수료를 합친 금액이 가용 자산보다 큽니다."
 *
 * 왜 그런가 (B건 탓이 아닙니다 — 순수 산수입니다)
 *   최대 버튼   margin = balance / (1 + 배율 × 0.0005)
 *   진입 검사   margin + margin × 배율 × 0.0005 > balance 이면 거절
 *   수학적으로는 margin × (1 + 배율 × 0.0005) = balance 라 딱 맞아야 하는데,
 *   나눈 뒤 다시 곱하면 ★마지막 자리에서 아주 조금 커질 수 있습니다.★
 *   지갑 130,000 · 50배에서 실제로 130000.00000000001 > 130000 이 됩니다.
 *   B건 이전에도 있던 성질입니다. 예전 100배 무대에서는 우연히 안 걸렸을 뿐입니다.
 *
 * ⚠️ 회원 눈에는 ★"최대 버튼을 눌렀는데 돈이 모자란다고 한다"★ 로 보입니다.
 *    PM 께 보고했습니다. 수리팀 B-2(js/qty-price-order.js 의 "최대(100%)" 버튼)
 *    에서 같이 볼 후보입니다.
 * ====================================================================== */
section("1-3. ⚠️ 발견 — 최대 버튼이 부동소수점으로 거절되는 지갑 (고쳐지면 여기가 터집니다)");
{
  const 열리나 = (지갑, 배율) => {
    const env = boot({ balance: 지갑 });
    env.tick(60000);
  ...
  ok("⚠️ 지갑 130,000 · 50배 는 지금 최대 버튼으로 진입이 거절된다 (아직 안 고쳐짐)",
    !x.열림,
    "열렸습니다 — ★고쳐진 것 같습니다.★ 축하합니다. 이 검사를 '열린다' 로 뒤집고 " +
      "위 주석의 발견 기록을 '고쳐짐(날짜)' 로 옮겨주세요.");
  ok("⚠️ 그 거절 사유는 구간 배율이 아니라 '가용 자산' 쪽이다 (원인 구분)",
    x.error.indexOf("가용 자산") >= 0, x.error);

  /* ★구간 상한 때문이 아니라는 증거★ — 같은 명목을 최대 버튼이 아니라
     손으로 조금 깎아 넣으면 그대로 열립니다. */
  const env = boot({ balance: 130000 });
  env.tick(60000);
  env.T.setLeverage(50);
  env.T.openPosition("long", env.T.getMaxAffordableMargin() - 0.01);
  ok("증거금을 0.01 만 깎으면 같은 지갑·같은 배율로 열린다 (구간 상한 문제가 아니다)",
    !!env.T.getSnapshot().position);
}
{
