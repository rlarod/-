...
/* 지금 무엇이 붙어 있나 — 사실을 적어 둡니다 */
{
  ...
  /* ⭐ 2026-09-03 (12.8절 · 차트팀) — MACD 가 틀로 들어왔습니다.
     이 파일이 "MACD 를 옮겼으면 이 줄만 고치면 됩니다" 라고 적어 둔 그 줄입니다.
     고친 것은 ★기대값 한 개★ 뿐이고 검사 논리는 그대로입니다.
     ⭐ 2026-09-03 (같은 날 뒤 · 차트팀) — Momentum(mom) 이 들어왔습니다.
        Mom = 지금 값 - n봉 전 값 이라 ★값이 가격 차이★ 입니다(MACD 와 같은 성질).
        위 [1] 의 재기가 비율 10.000 으로 「가격」 이라 판정했고, [2] 의 「필수」
        규칙이 unit:"price" 를 요구했습니다. ★검사 논리는 그대로이고 기대값
        한 줄만★ atr,macd → atr,macd,mom 으로 옮겼습니다.
        ⚠️ 같이 온 ROC 는 %(지수)라 비율 1.000 — unit 을 안 붙였습니다.
     ⭐ 2026-09-03 (14단계 · 차트팀) — MFI · CMF · TRIX · AO 넷이 들어왔습니다.
        PM 지시서에는 「넷 다 지수라 unit 을 붙이지 마라」 고 적혀 있었는데,
        위 [1] 의 재기가 ★AO 만 비율 10.000(가격)★ 이라고 판정했습니다 —
        AO = 중간값 단순평균(5) − 단순평균(34) 이라 MACD 와 뿌리가 같습니다.
        Momentum 때와 ★같은 일★ 이 또 났고, 재 본 결과를 따랐습니다.
        MFI · CMF · TRIX 는 비율 1.000 이라 안 붙였습니다.
        ★검사 논리는 그대로이고 기대값 한 줄만★ atr,macd,mom → atr,macd,mom,ao. */
  ok('unit:"price" 가 붙은 것은 지금 [' + 붙은것.join(" ") + "] 넷이다 (ATR · MACD · Momentum · AO)",
    붙은것.join(",") === "atr,macd,mom,ao",
    "지금: " + 붙은것.join(",") + "  → 새 지표를 옮겼으면 이 줄만 고치면 됩니다. " +
...

{
  ...
  ok("기준선이 있는 지표 " + 기준선지표.length + "개 중 눈금을 고정한 것은 [" + 눈금건것.join(" ") + "] 이다",
    눈금건것.join(",") === "mfi,rsi,srsi,stoch,wr",
    "지금: " + 눈금건것.join(",") +
  ...
  /* ⚠️ 눈금을 ★안★ 고정한 채 기준선만 있는 지표는 추세장에서 기준선이 사라집니다.
     ...
     ⭐ 2026-09-03 차트팀이 고쳤습니다. ★검사 논리는 한 글자도 안 바꿨고★
        기대값 세 줄만 새 사실로 옮겼습니다.
          srsi · stoch  scale { min:0, max:100 } 을 걸었습니다 (RSI 와 같음)
                        → 위 「눈금을 고정한 것」 목록이 rsi → rsi,srsi,stoch
          cci           0~100 짜리가 아니라 ★고정하면 봉우리가 잘립니다.★
     ...
     ⭐ 2026-09-03 (같은 날 뒤) 차트팀이 ROC(roc) 를 얹었습니다. 트레이딩뷰 도움말이
        "it is not bounded to a set range" 라고 못 박아 둔 지표라 min/max 로 고정하면
        봉우리가 잘립니다. cci 와 ★같은 길★ — scale { keepGuides:true } 를 걸었습니다.
        그래서 아래 「데이터만으로는 밖」 목록에 cci 옆에 roc 가 늘었습니다.
        ★검사 논리는 그대로이고 기대값 한 줄만★ cci → cci,roc 로 옮겼습니다.
     ⭐ 2026-09-03 (14단계) 차트팀이 MFI · CMF · TRIX · AO 를 얹었습니다.
        MFI  정의상 0~100 짜리라 scale { min:0, max:100 } 을 걸었습니다(RSI 와 같음).
             기준선 20 · 80 이 그 안이라 절대 화면 밖으로 못 나갑니다.
             → 위 「눈금을 고정한 것」 목록에 mfi 가 늘었습니다.
        CMF · TRIX · AO  범위가 정해진 지표가 아니라 고정하면 봉우리가 잘립니다.
             cci · roc 와 ★같은 길★ — scale { keepGuides:true } 를 걸었습니다.
             → 아래 「데이터만으로는 밖」 목록에 ao · cmf · trix 가 늘었습니다.
        ★검사 논리는 두 줄 다 그대로이고 기대값만★ 새 사실로 옮겼습니다.
     ⚠️ 그래서 아래 재기에는 cci 가 ★그대로 남습니다★ — 이 재기는 「그린 값의
        ...
        tests/chart-indicator-pane-kit.test.js 쪽 provider 검사가 봅니다.
        이 줄은 「데이터만으로는 밖」 인 지표를 세는 줄로 읽으세요. */
  const 상승봉 = [];
  {
    let px = 70000;
    for (let i = 0; i < 봉수; i++) {
