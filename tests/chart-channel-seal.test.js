 ...
 *   [6] 안내 한 줄이 접히지 않는가
 *        차트팀 실측 — 360 에서 폭 277px · 높이 41px (한 줄)
 *        (2026-09-02 15차에 알림줄 글씨를 12px -> 17px 로 올렸습니다.
 *         그전 실측은 27px 이었습니다. 대표가 작은 글씨를 못 읽습니다)
 ...
 * ========================================================================= */
절("[6] 안내 한 줄이 접히지 않는다 (차트팀 실측 360 에서 41px)");
{
  ...
  /* nowrap 을 걸지 않습니다 — placeToast() 가 카드 오른쪽 끝에 맞춰 밀어
     ...
     15차 2026-09-02 — 알림줄 글씨를 12px -> 17px 로 올렸습니다(대표 지시:
     "대표가 작은 글씨를 못 읽는다"). 그래서 아래 숫자가 전부 바뀌었습니다.
     [주의] 여기에 크기를 손으로 또 적지 않습니다. 모듈이 화면에 실제로 넣은
     <style> 을 읽습니다 — 소스 글자만 보면 "font-size:" + TOAST_FONT + "px"
     같은 이어붙인 값을 못 읽고 조용히 NaN 으로 통과해 버립니다. */
  const 스타일칸 = t.win.document.getElementById("chart-drawings-style");
  ok("알림줄 CSS 를 화면에 실제로 넣었다", !!스타일칸 && 스타일칸.textContent.length > 0);
  const cssAll = 스타일칸 ? 스타일칸.textContent : "";
  const css = cssAll.slice(cssAll.indexOf(".tl-draw-toast{"), cssAll.indexOf(".tl-draw-toast{") + 400);
  ...
  const 가로여백 = parseFloat((css.match(/padding:\d+(?:\.\d+)?px (\d+(?:\.\d+)?)px/) || [])[1]);
  ok("안내 글자 크기를 실제로 읽었다", 글자 > 0 && 줄높이 > 0 && 세로여백 > 0 && 가로여백 > 0,
    글자 + " / " + 줄높이 + " / " + 세로여백 + " / " + 가로여백);
  ok("안내 글자가 모듈이 정한 크기(TOAST_FONT)와 같다 — 적는 곳은 한 곳뿐이다",
    글자 === t.M.TOAST_FONT, 글자 + " vs " + t.M.TOAST_FONT);
  ok("안내 글자가 15px 아래로 내려가지 않는다 (대표가 작은 글씨를 못 읽습니다)",
    글자 >= 15 && 글자 >= t.M.TOAST_MIN_FONT, String(글자));
  ok("안내 글자가 OHLC 범례와 같은 17px 이다", 글자 === 17, String(글자));

  const 한줄 = 글자 * 줄높이 + 세로여백 * 2 + 2;   /* 글자줄 + 여백 + 테두리 1px 두 줄 */
  ok("한 줄 높이 계산이 차트팀 실측 41px 과 맞는다 (17 x 1.45 + 7 x 2 + 1 x 2 = 40.7)",
    Math.round(한줄) === 41, 한줄.toFixed(1) + "px");
  ok("두 줄이면 65px 이라 41 과 확실히 구분된다 (높이만 봐도 접힘을 알 수 있습니다)",
    Math.round(글자 * 줄높이 * 2 + 세로여백 * 2 + 2) === 65,
    String(Math.round(글자 * 줄높이 * 2 + 세로여백 * 2 + 2)));
  ...
  /* 글이 길어지면 접힙니다. 좁은 폭에서 들어가는지 글자 수로 미리 잽니다.
     ...
     진짜 폭은 차트팀이 360 크롬에서 재서 폭 277px · 높이 41px 한 줄을 확인했습니다
     (2026-09-02 15차. 12px 때는 27px 이었습니다). */
  const 어림폭 = (s) => {
    let w = 0;
    for (const ch of s) {
      if (/[가-힣ㄱ-ㆎ一-鿿—]/.test(ch)) w += 글자;
      else if (ch === " ") w += 글자 * 0.28;
      else w += 글자 * 0.55;
    }
    return w;
  };
  const 안내폭 = 어림폭("세 번 톡 — 기준선 두 점, 그 다음 폭") + 가로여백 * 2 + 2;
  ...
  /* 글씨를 키운 만큼 여유가 줄었습니다.
       어림(넉넉하게 잡은 값) 299 / 314 = 95%
       크롬 실측(360)        277 / 314 = 88%   <- 진짜 값
     [주의] 여유가 얼마 없습니다. 「안내 글을 더 길게 쓰면 두 줄이 됩니다.」
     그때는 글씨를 줄이지 말고 글을 짧게 쓰거나 두 줄을 그대로 두세요. */
  ok("한 줄 여유가 남아 있다 (어림이 쓸 수 있는 폭 안쪽)",
    안내폭 <= 쓸수있는폭, Math.round((안내폭 / 쓸수있는폭) * 100) + "%");
  ok("안내 글이 지금보다 길어지면 두 줄이 된다는 것을 알고 있다 (어림 여유 " +
    Math.round(쓸수있는폭 - 안내폭) + "px)",
    쓸수있는폭 - 안내폭 >= 0 && 쓸수있는폭 - 안내폭 < 글자 * 2,
    "여유 " + Math.round(쓸수있는폭 - 안내폭) + "px (한 글자 " + 글자 + "px)");
  t.닫기();
}
{
