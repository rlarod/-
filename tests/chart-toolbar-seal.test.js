...
/* ★2026-09-07 (23차) — 여기 두 줄이 ★뒤집혔습니다★ (차트팀).
 *   전 : 폰 가로 막대는 flex-wrap:wrap · overflow:visible (넘치면 두 줄)
 *   후 : 폰 가로 막대는 flex-wrap:nowrap · overflow-x:auto (넘치면 옆으로 밀기)
 *
 *   근거 세 가지 — 숫자를 남깁니다.
 *   (가) PM 지시 2026-09-07 — "넘칠 때 두 줄로 늘리지 마세요.
 *        트레이딩뷰는 390 에서도 한 줄이고 가로 스크롤입니다."
 *   (나) 트레이딩뷰 실측 (2026-09-07 · 1440x900) — 맨 윗줄 52,0 1388x38 한 줄.
 *        390 에서도 h38 ★한 줄★ 이고 x=366 이후 가로 스크롤. 두 줄이 없습니다.
 *   (다) 이 막대는 이제 차트 칸이 아니라 ★페이지 폭★ 에 있고, 시간 단위 줄까지
 *        같이 들어옵니다(js/chart-toprow.js). 접기로는 폰에서 4~5줄이 됩니다.
 *        localhost 실측 — 수정 전 360/375/390 은 시간단위 95px(2줄) +
 *        도구막대 89px(2줄) = ★184px★. 합쳐서 한 줄로 하면 ★44px★ 입니다.
 *
 *   ⚠ 위 (2) 항의 걱정("오버레이 스크롤바라 밀 수 있는 줄 모른다")은
 *     ★세로 막대에는 그대로 살아 있습니다★ — 바로 위 .tlc-rail 검사 넷이
 *     한 글자도 안 바뀌었습니다. 뒤집은 것은 가로 막대뿐입니다.
 *     가로 막대는 밀 수 있다는 신호를 두 가지로 줍니다 —
 *       ① 3px 얇은 스크롤바(#1D273B) 를 폰에서도 그대로 씁니다
 *       ② js/chart-toprow.js 가 오른쪽 끝에 › 힌트를 얹습니다(넘칠 때만)
 *   되돌리려면 이 토막을 옛 두 줄(wrap · visible)로 되돌리고
 *   css/chart-toolbar.css 6) 의 .tlc-toolbar 도 같이 되돌립니다. */
ok("★폰 .tlc-toolbar 가 한 줄이다★ (flex-wrap:nowrap — 두 줄로 늘리지 않습니다)",
  !!mBar && /flex-wrap\s*:\s*nowrap/.test(mBar), flat(mBar));
ok("★폰 .tlc-toolbar 가 넘치면 옆으로 민다★ (overflow-x:auto — 트레이딩뷰와 같은 방식)",
  !!mBar && /overflow-x\s*:\s*auto/.test(mBar), flat(mBar));
ok("폰 .tlc-toolbar 에 overflow:visible 이 남아 있지 않다 (남으면 overflow-x 를 덮어 두 줄이 됩니다)",
  !!mBar && !/overflow\s*:\s*visible/.test(mBar), flat(mBar));
ok("폰 .tlc-toolbar 키가 한 줄로 못 박혀 있다 (height:var(--tlc-bar-h-m))",
  !!mBar && /height\s*:\s*var\(--tlc-bar-h-m\)/.test(mBar), flat(mBar));
ok("왜 뒤집었는지가 CSS 주석에 숫자로 남아 있다 (다음 사람이 '누가 몰래 바꿨나' 로 읽지 않게)",
  /390 에서도 한 줄/.test(CSS_RAW) && /184px/.test(CSS_RAW),
  "css/chart-toolbar.css 주석에서 못 찾음");
     ...
     2026-08-26 처럼 폰/데스크톱이 갈린 뒤로는 틀린 답이 나옵니다. */
{
  ...
  /* ★2026-09-07 (23차) — "몇 줄이 되나" 검사를 ★"한 줄인가"★ 로 바꿨습니다.
   *   가로 막대는 이제 접히지 않습니다(flex-wrap:nowrap). 넘치면 옆으로 밉니다.
   *   그래서 "두 줄까지 허용 / 세 줄은 금지" 라는 옛 잣대가 뜻을 잃었습니다.
   *   대신 ★단추가 아무리 늘어도 늘 한 줄★ 이라는 것을 못 박습니다.
   *
   *   옛 검사를 그냥 지우지 않고 뒤집은 이유 — 지우면 "가로 막대가 회원 화면을
   *   세로로 얼마나 먹는가" 를 아무도 안 보게 됩니다. 그 값이 이 파일이 지키던
   *   진짜 대상입니다. 이제 그 값은 ★한 줄 키(--tlc-bar-h-m = 44px)★ 하나입니다.
   *
   *   실측 (localhost 2026-09-07, 합친 뒤)
   *     360x800 · 375x800 · 390x800 · 360x640   막대 44px ★1줄★ (단추 17개)
   *     768 · 800x360                           막대 40px ★1줄★
   *     1440 · 1920                             막대 40px ★1줄★ · 넘침 0
   *   되돌리려면 이 토막을 옛 두 검사(줄수<=2 · 마지막줄>=2)로 되돌립니다. */
  ok("★단추가 몇 개든 가로 막대는 한 줄이다★ (넘치면 옆으로 밉니다 — 두 줄이 안 됩니다)",
    /flex-wrap\s*:\s*nowrap/.test(mBar || ""), flat(mBar));
  ok("한 줄 키가 폰 버튼 한 칸과 같다 (막대가 회원 화면에서 먹는 세로는 " + M_BAR_H_M + "px 하나뿐)",
    M_BAR_H_M === M_BTN, M_BAR_H_M + " vs " + M_BTN);
  /* 옛 셈은 그대로 남겨 「접었다면 몇 줄이었을지」를 보고만 합니다.
     이 숫자가 커질수록 옆으로 미는 거리가 길다는 뜻입니다(막는 기준은 아닙니다). */
  console.log("      (참고 — 360 에서 접는 방식이었다면 가로 " + 줄수(가로단추) +
    "줄 · 세로 " + 줄수(세로단추) + "줄이었을 것입니다. 지금은 가로 1줄입니다)");
