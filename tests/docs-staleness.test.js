...
/* 지금 남아 있는 어긋남. 본부장이 고쳐야 할 목록입니다.
 * 고치면 이 줄을 지우세요. 여기 없는 것이 새로 생기면 실패합니다.
 *
 * 2026-08-25  6건으로 시작
 * 2026-08-28  백로그가 정리돼서 3건을 지웠습니다(감사팀 확인 → 본부장 지시).
 *             지운 것 — "TL-004 | 준비됨, 미적용" · "TL-004 | 미적용" · "TL-006 | 아직 안 함"
 *             이 세 문구가 다시 나타나면 이제 "새로 생긴 어긋남" 으로 걸립니다.
 *             남은 3건은 아직 실재해서 그대로 둡니다. */
const KNOWN_어긋남 = [
  "TL-001 | 착수하지 않습니다",
  "TL-002 | 뒤로 미룬다",
  "TL-005 | 고치면 같이 해결됩니다",
...
/* 결번 예외 — 지금은 하나도 없습니다(빈 목록).
 *
 * 2026-08-25 에는 [4] 가 들어 있었습니다. 통화 버튼 항목(4번)을 지우면서
 * 결번이 생겼기 때문입니다.
 * 2026-08-26 본부장이 승인대기.md 를 1~7 로 다시 매겨 결번이 없어졌습니다.
 * 그런데 예외 [4] 를 그대로 두면 "4번이 통째로 사라져도 조용히 통과"하는
 * 구멍이 남습니다. 실제로 지금 4번은 "로그인한 폰 화면 캡처"라는 살아 있는
 * 항목이라, 없어지면 반드시 걸려야 합니다. 그래서 예외를 지웠습니다.
 *
 * 앞으로도 항목을 지울 때는 번호를 다시 매기고, 여기에 예외를 추가하지 않습니다.
 * (예외를 늘리는 순간 이 검사는 아무것도 안 지킵니다) */
const KNOWN_결번 = [];
