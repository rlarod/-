 ...
 *   "한 줄에 몇 개" 를 정하고 실측 높이(1줄 23 · 2줄 49 · 3줄 76)를 흉내 냅니다.
 *
...
/* 실측 — 한 줄 23px, 두 줄 49px, 세 줄 76px (1440/360 둘 다 같았습니다) */
function 높이(줄수) {
  return 줄수 <= 0 ? 0 : 23 + (줄수 - 1) * 26.5;
}
...
(async function () {
  ...
  절("[1] 세 줄이면 처음부터 접힘");
  {
    ...
    ok("접혀 있다", s.접힘 === true);
    ok("한 줄로 줄었다 (" + s.높이 + "px, 세 줄이면 76px)", s.높이 === 높이(1), String(s.높이));
