 ...
 * 2026-08-27 대표 지시로 사업본부장 직속 "감사팀"(audit-team) 이 생겨 7팀이 됐습니다.
 ...
 *    감사팀은 감시 → 확인 → 판단 → 본부장 보고까지만 하고, 업무 배정은 오직
 *    사업본부장이 합니다. 그 제약이 지침에서 조용히 빠지는 것을 (11) 이 막습니다.
 *
...
/* 실무 6팀 — 파일명(=name) 과 한글 이름. 본부장 아래 가로로 늘어선 팀들 */
const TEAMS = [
  { slug: "qa-team",     ko: "점검팀" },
...
/* 2026-08-27 신설 — 감사팀은 실무 6팀과 나란히 있지 않고 사업본부장 직속입니다.
...
console.log("\n(6) CLAUDE.md 와 README.md 가 같은 7팀을 적는다");
{
  ...
  /* 감사팀은 실무 6팀 표가 아니라 조직도 쪽에 들어갔습니다(본부장 직속) */
  ok("README.md 조직도에 감사팀이 본부장 직속으로 그려져 있다",
    /사업본부장[^\n]*감사팀/.test(rm));
  ...
  ok("CLAUDE.md 조직도에 감사팀이 본부장 직속으로 그려져 있다",
    /사업본부장[^\n]*감사팀/.test(claude));
 ...
 *      그러면 "본부장이 재현하고 본부장이 검증한다" 는 구조 자체가 무너집니다.
 *
...
console.log("\n(11) 감사팀 제약 -- 상사가 아니다 / 일을 만들지 않는다");
{
  ...
  /* --- 11-1 위치: 사업본부장 직속이다 ------------------------------- */
  ok("감사팀이 사업본부장 직속이라고 적혀 있다", /사업본부장 직속/.test(a));
  ok("description 에도 직속이라고 적혀 있다", /사업본부장 직속/.test(desc));

  /* --- 11-2 [핵심] 지시·배정을 하지 않는다 -------------------------- */
  ok("[핵심] '다른 팀의 상사가 아니다' 가 살아 있다", /다른 팀의 상사가 아니다/.test(a));
  ok("[핵심] '업무 배정은 오직 사업본부장이' 라는 문장이 살아 있다",
    /업무 배정은[\s\S]{0,20}오직[\s\S]{0,20}사업본부장이/.test(a),
  ...
  ok("역할이 '감시 → 확인 → 판단 → 보고' 로 끝난다고 적는다",
    /감시\s*→\s*확인\s*→\s*판단\s*→[^\n]*보고/.test(a));
  ok("나쁜 예/좋은 예로 '팀에게 말하지 말고 본부장에게 보고' 를 보여준다",
    /나쁜 예[^\n]*고쳐/.test(a) && /좋은 예[^\n]*본부장에게/.test(a));
  ...
  ok("게이트 1(본부장 재현) 과 게이트 2(본부장 검증) 를 둘 다 적는다",
    /게이트 1[^\n]*재현/.test(a) && /게이트 2[^\n]*검증/.test(a));
