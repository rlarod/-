 ...
 * ───────────────────────────────────────────────────────────────────── */
/* ── 2026-09-02 (11단계) 2 → 5 로 올렸습니다. ★근거를 적습니다★ ──────────
 *   ⚠️ ★기본으로 켜지는 지표를 늘린 것이 아닙니다.★ 다섯 줄 전부 꺼짐이고,
 *      그것은 아래 "둘 다 꺼져 있다" 검사가 그대로 지키고 있습니다.
 *   무엇이 늘었나 — 옛 js/chart-indicators.js 가 그리던
 *      MA(7) #F0B429 · MA(25) #E7ECF5 · MA(99) #838DA4
 *   세 줄이 ★이 틀로 옮겨 왔습니다★(js/chart-indicator-kit.js 12.5절).
 *   왜 옮겼나 — 조사팀 [A] 최우선. fx 목록 9줄 중 설정 버튼이 붙은 줄이
 *      2줄뿐이었고, MA 세 줄은 기간·색·굵기가 코드에 박혀 회원이 하나도
 *      못 바꿨습니다. 틀로 옮기면 트레이딩뷰처럼 전부 바꿉니다.
 *   회원이 보는 줄 수는 ★그대로★ 입니다 — 옛 세 줄을 fx 목록과 칩 줄에서
 *      빼고 그 자리에 새 세 줄을 끼웁니다(순서까지 같습니다).
 *   ⚠️ 이 봉인 자체는 그대로입니다. 다음에 또 늘리려면 여기에 또 근거를 적으세요.
 * ─────────────────────────────────────────────────────────────────────── */
/* ── 2026-09-02 (12단계) 5 → 6 으로 올렸습니다. ★근거를 적습니다★ ──────────
 *   ⚠️ ★기본으로 켜지는 지표를 늘린 것이 아닙니다.★ 여섯 줄 전부 꺼짐이고,
 *      그것은 아래 "둘 다 꺼져 있다" 검사가 그대로 지키고 있습니다.
 *   무엇이 늘었나 — 옛 js/chart-indicators.js 가 그리던
 *      볼린저밴드 BOLL(20, 2) #838DA4 점선 세 줄(위·중간·아래)이
 *      ★이 틀로 옮겨 왔습니다★(js/chart-indicator-kit.js 12.6절).
 *      회원에게는 스위치 하나라 인스턴스도 ★하나★(bb-20)입니다.
 *   왜 옮겼나 — MA 를 옮긴 것과 같은 이유입니다. 옛 볼린저는 기간 20 ·
 *      배수 2 · 색 · 점선이 전부 코드에 박혀 회원이 하나도 못 바꿨습니다.
 *   회원이 보는 줄 수는 ★그대로★ 입니다 — 옛 BOLL 줄을 fx 목록과 칩 줄에서
 *      빼고 ★그 자리에★ 새 줄을 끼웁니다(MA(99) 다음, VOL 앞).
 *   안 바꾼 것 — 색 #838DA4 셋 · 점선 · 굵기 1 · 기간 20 · 배수 2 ·
 *      ★계산값(옛 computeBB 와 오차 0)★.
 *   ⚠️ 이 봉인 자체는 그대로입니다. 다음에 또 늘리려면 여기에 또 근거를 적으세요.
 * ─────────────────────────────────────────────────────────────────────── */
const 기본인스턴스수 = 6;
...
console.log("\n[1] 지표선 색 목록 (LINE_COLORS)");
{
  ...
  /* ⚠️ 2026-09-02 에 12 -> 20, ★2026-09-04 에 20 -> 27★ 로 늘렸습니다.
     개수를 여기 박아 두는 이유는 "몇 개인지" 가 중요해서가 아니라 ★말없이 줄거나
     느는 것★ 을 잡으려는 것입니다. 늘릴 때는 아래 거리 검사를 같이 통과해야 합니다.

     ── 27 로 늘린 근거 (다음 사람이 "누가 몰래 늘렸나" 로 읽지 않게) ──────
       대표 승인   2026-09-02. PM 질문 "차트 지표에 한해 색을 새로 만들어도
                   될까요? (팔레트 자체는 안 건드리고, 지표용 색만 따로)"
                   대표 답변 원문 ★"ㅇㅋ"★.
                   CLAUDE.md "⭐ 예외 — 차트 지표 색은 늘려도 됩니다" 에 그대로 있습니다.
       PM 판단     2026-09-04. 20 -> 27 은 PM 이 정했습니다. 근거는
                   ★트레이딩뷰가 지표선을 전부 실선으로 그린다★ 는 것입니다.
                   색이 모자란다고 점선 · 파선으로 가르면 트레이딩뷰와 달라집니다
                   (CLAUDE.md - 차트 시스템 전체는 트레이딩뷰를 따라간다).
                   그래서 구분은 색으로만 하고, 색을 늘렸습니다.
       ⚠️ 확정 팔레트 9색은 ★한 값도 안 바꿨습니다.★ 늘어난 것은 지표선 목록뿐입니다.
       ⚠️ 앞 20색의 ★값과 순서도 그대로★ 입니다 - 바꾸면 이미 저장된 회원의
          지표 색이 목록 밖 값이 되어 거부됩니다.
       고른 방법과 실측 거리는 js/chart-indicator-kit.js 의 LINE_COLORS 주석에 있습니다.
       (얹지 않고 27색 전체로 maximin 을 다시 뽑았습니다 - 그냥 얹으면
        최소 ΔE2000 이 9.71 -> 8.83 으로 떨어집니다) */
  ok("색이 27개다", hexes.length === 27, "지금 " + hexes.length + "개");
  ok(
    "전부 #RRGGBB 형식이다",
...
console.log("\n[3] 인스턴스 (정의 1개 + 인스턴스 N개)");
{
  ...
  /* ⭐ 이 틀의 뿌리 — "정의 1개 + 인스턴스 N개".
     MA 정의 ★하나★ 에서 7 · 25 · 99 세 줄이, EMA 정의 ★하나★ 에서 두 줄이
     나옵니다. 정의가 인스턴스 수만큼 늘어나면 여기가 터집니다. */
  const 정의별 = {};
  목록.forEach((i) => {
    정의별[i.def] = (정의별[i.def] || 0) + 1;
  });
  /* ⭐ 2026-09-02 (12단계) 에 "ema,ma" → "bb,ema,ma" 로 늘렸습니다.
     옛 볼린저가 이 틀로 옮겨 왔습니다(위 기본인스턴스수 주석의 근거와 같은 건).
     ⚠️ 뜻은 그대로입니다 — ★정의 수 < 인스턴스 수★. 지금 정의 3 · 인스턴스 6. */
  ok(
    "정의는 셋뿐이다 (ma · bb · ema)",
    Object.keys(정의별).sort().join(",") === "bb,ema,ma",
    JSON.stringify(정의별)
  );
  ok("MA 정의 하나에서 세 줄이 나왔다", 정의별.ma === 3, JSON.stringify(정의별));
  ok("EMA 정의 하나에서 두 줄이 나왔다", 정의별.ema === 2, JSON.stringify(정의별));
  ok("볼린저 정의 하나에서 한 줄이 나왔다 (선은 셋이지만 스위치는 하나)", 정의별.bb === 1, JSON.stringify(정의별));
  ok("정의 수가 인스턴스 수보다 적다", Object.keys(정의별).length < 목록.length,
    Object.keys(정의별).length + " < " + 목록.length);
  ok(
    "전부 꺼져 있다 (기본은 전부 꺼짐)",
  ...
  /* ⚠️ 옮겨 온 MA 세 줄의 ★색·기간★ — 대표가 매일 보시던 그대로여야 합니다.
     하나라도 바뀌면 회원 화면에서 선 색이 갑자기 달라집니다. */
  const 옛MA = { "ma-7": ["#F0B429", 7], "ma-25": ["#E7ECF5", 25], "ma-99": ["#838DA4", 99] };
  /* 옮겨 온 볼린저도 같은 이유로 못 박습니다 — 색 셋 다 #838DA4 · 점선 · 20 · 2 */
  {
    const bb = 목록.filter((i) => i.id === "bb-20")[0];
    ok("옮겨 온 bb-20 이 있다", !!bb);
    ok(
      "bb-20 의 위·중간·아래가 셋 다 #838DA4 다",
      !!bb && bb.colors.upper === "#838DA4" && bb.colors.middle === "#838DA4" && bb.colors.lower === "#838DA4",
      bb ? JSON.stringify(bb.colors) : "없음"
    );
    ok("bb-20 이 점선이다", !!bb && bb.style === "dashed", bb ? bb.style : "없음");
    ok(
      "bb-20 이 기간 20 · 배수 2 다",
      !!bb && bb.params.p === 20 && bb.params.k === 2,
      bb ? bb.params.p + " / " + bb.params.k : "없음"
    );
  }
  Object.keys(옛MA).forEach((id) => {
    const it = 목록.filter((i) => i.id === id)[0];
    ok(
      "옮겨 온 " + id + " 이 옛 색·기간 그대로다 " + 옛MA[id][0] + " / " + 옛MA[id][1],
      !!it && it.colors.ma === 옛MA[id][0] && it.params.p === 옛MA[id][1],
      it ? it.colors.ma + " / " + it.params.p : "없음"
    );
  });
...
console.log("\n[7] EMA 값 대조 (전체 재계산과 같은가)");
{
  ...
  /* ⚠️ 11단계에서 목록 앞자리가 MA 로 바뀌었습니다 — 자리(0·1)가 아니라
     ★정의★ 로 골라야 무엇을 재는지가 안 흔들립니다. */
  const 이엠에이 = K.listInstances().filter((i) => i.def === "ema");
  ok(
    "두 선의 색이 서로 다르다",
    이엠에이.length === 2 && 이엠에이[0].colors.ema !== 이엠에이[1].colors.ema
  );
