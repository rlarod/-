...
(async function run() {
  ...
  {
    const { K } = boot({});
    ok("카카오 회원을 알아본다", K.isSocialUser(kakaoUser()));
    ok("닉네임+비밀번호 회원은 카카오가 아니다",
      !K.isSocialUser({ id: "x", app_metadata: { provider: "email" }, identities: [{ provider: "email" }] }));
    ok("여러 방식을 연결한 계정도 알아본다",
      K.isSocialUser({ id: "x", app_metadata: { provider: "email" }, identities: [{ provider: "email" }, { provider: "kakao", provider_id: "1" }] }));
    ok("카카오 회원번호를 꺼낸다", K.socialUserId(kakaoUser()) === "9876543210");
    ok("카카오 회원번호가 없으면 지어내지 않는다", K.socialUserId({ id: "x", identities: [] }) === null);
  }

  /* ---------- 카카오 닉네임 제안 ---------- */
  {
    const { K } = boot({});
    ok("규칙에 맞는 카카오 닉네임은 제안한다", K.suggestedNickname(kakaoUser()) === "카카오닉");
    ok("규칙에 어긋나면 제안하지 않는다(억지로 고치지 않음)",
      K.suggestedNickname(kakaoUser({ user_metadata: { name: "★관리자★" } })) === "");
    ok("공백이 든 카카오 닉네임도 제안하지 않는다",
      K.suggestedNickname(kakaoUser({ user_metadata: { name: "김 갱" } })) === "");
    ok("카카오가 이름을 안 줘도 안 터진다", K.suggestedNickname(kakaoUser({ user_metadata: {} })) === "");
  }

  /* ---------- auth.js 를 감쌌는지 ---------- */
  {
    const { win } = boot({});
    ok("auth.js 의 init 을 감쌌다", win.App.Auth.__socialWrapped === true);
  ...
  /* ---------- 순서: 닉네임이 저장된 다음에 auth.js 가 시작된다 ---------- */
  {
    ...
    ok("신규 회원이면 닉네임 화면이 먼저 뜬다", !!doc.getElementById("social-nick-gate"));
    ok("닉네임을 받기 전에는 auth.js 가 시작되지 않는다",
      calls.filter((c) => c[0] === "auth.init").length === 0,
      "auth.init 이 먼저 불리면 세션이 로그아웃됩니다");

    doc.getElementById("social-nick-input").value = "김갱";
    doc.getElementById("social-nick-submit").click();
    await sleep(50);
    ...
    ok("닉네임 화면이 닫힌다", !doc.getElementById("social-nick-gate"));
  }
  ...
  /* ---------- 기존 회원은 방해하지 않는다 ---------- */
  {
    ...
    win.App.Auth.init();
    await sleep(50);
    ok("닉네임이 있는 카카오 회원에게는 화면을 띄우지 않는다", !doc.getElementById("social-nick-gate"));
  ...
  }
  {
    ...
    win.App.Auth.init();
    await sleep(50);
    ok("닉네임+비밀번호 회원에게는 끼어들지 않는다", !doc.getElementById("social-nick-gate"));
  ...
  }
  {
    ...
    ok("로그인 안 한 사람에게는 아무것도 하지 않는다",
      !doc.getElementById("social-nick-gate") && calls.some((c) => c[0] === "auth.init"));
  }
  ...
  /* ---------- 닉네임 규칙은 기존 규칙 그대로 ---------- */
  {
    ...
    let blocked = 0;
    for (const n of bad) {
      doc.getElementById("social-nick-input").value = n;
      doc.getElementById("social-nick-submit").click();
      await sleep(10);
      if (!calls.some((c) => c[0] === "insert")) blocked++;
    }
    ok("규칙에 어긋난 닉네임은 저장 시도조차 안 한다", blocked === bad.length, blocked + "/" + bad.length);
    ok("막힌 뒤에도 화면은 열려 있다", !!doc.getElementById("social-nick-gate"));
    ok("이유를 알려준다", (doc.getElementById("social-nick-err").textContent || "").length > 0);
  }
  ...
  /* ---------- 로그인 버튼 ---------- */
  {
    ...
    ok("로그인 폼 안에 카카오 버튼이 있다", !!btn && !!doc.querySelector(".user-panel-guest #social-login-kakao"));
  ...
  }
  {
    ...
    await sleep(20);
    doc.getElementById("social-login-kakao").click();
    await sleep(30);
  ...
  /* ---------- 폼이 다시 그려져도 버튼이 살아있다 ---------- */
  {
    ...
    ok("다시 그린 직후에는 버튼이 없다", !doc.getElementById("social-login-kakao"));
    K.injectButton();
    ok("다시 넣으면 살아난다", !!doc.getElementById("social-login-kakao"));
    K.injectButton();
  ...
  }
  {
    ...
    const { K } = boot({});
    ok("네이버 회원을 알아본다", K.isSocialUser(naverUser));
    ok("네이버 회원번호를 꺼낸다", K.socialUserId(naverUser) === "naver-123");
    ok("네이버가 준 닉네임을 제안한다", K.suggestedNickname(naverUser) === "네이버닉");
    ok("어느 수단으로 들어왔는지 안다", K.socialProviderOf(naverUser) === "custom:naver");
  }
  {
    const naverUser = {
      id: "u9", email: "a@naver.com",
      app_metadata: { provider: "custom:naver" },
      user_metadata: { nickname: "네이버닉" },
      identities: [{ provider: "custom:naver", provider_id: "naver-123" }],
    };
    const { win, doc, calls } = boot({ session: { user: naverUser }, profile: null });
    win.App.Auth.init();
    await sleep(50);
    ok("네이버 신규 회원도 닉네임 화면이 뜬다", !!doc.getElementById("social-nick-gate"));
    doc.getElementById("social-nick-input").value = "김갱";
    doc.getElementById("social-nick-submit").click();
    await sleep(50);
    const ups = calls.find((x) => x[0] === "upsert");
    /* 'custom:' 은 Supabase 쪽 접두어입니다. 마이페이지는 'naver' 를 보고
       '네이버' 라고 표시하므로, 접두어를 떼고 저장해야 합니다. */
    ok("개인정보에는 'naver' 로 남긴다(custom: 접두어 제거)",
      ups && ups[2].provider === "naver", ups && ups[2].provider);
    ok("네이버 회원번호도 남긴다", ups && ups[2].provider_user_id === "naver-123");
  }
  ...
  /* ---------- 수정 금지 파일 ---------- */
  {
    ...
    const kakaoAt = html.indexOf('js/social-login.js');
    ...
    ok("카카오 버튼 폭을 100% 로 잡았다(로그인 버튼과 단차 방지)",
      /\.social-login-wrap\{[^}]*width:100%/.test(css));
